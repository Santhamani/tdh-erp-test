...
export const SalesActivityTable: React.FC<{ currentUser: User }> = ({ currentUser }) => {
    const { logs } = useAuth();
    ...
    const filteredLogs = useMemo(() => {
        ...
        const salesLogs = (logs || [])
            .filter(log => log.stageId === 'sales' && (isManager || log.userId === currentUser.id))
        ...
        return salesLogs.filter(log => {
            ...
            const rowCount = String((details.rows || []).length || 0);

            return (
                fileName.toLowerCase().includes(lowercasedSearch) ||
                rowCount.includes(lowercasedSearch)
            );
        });
    }, [logs, currentUser.id, currentUser.role, searchTerm, timeFilter, customStartDate, customEndDate]);
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md w-full">
            <h3 className="text-xl font-semibold text-slate-700 mb-4">Sales Activity</h3>
            <div className="flex flex-col md:flex-row justify-between items-center mb-4 space-y-4 md:space-y-0">
                <div className="relative w-full md:max-w-xs">
                    <input
                        type="text"
                        placeholder="Search by file, rows..."
                        value={searchTerm}
            ...
            {timeFilter === 'custom' && (
                <div className="flex flex-col sm:flex-row items-center gap-2 mb-4 p-2 bg-slate-50 rounded-lg justify-center">
                    <label htmlFor="start-date-sales" className="text-sm font-medium text-slate-600">From:</label>
                    <input
                        id="start-date-sales"
                        type="date"
                    ...
                    />
                    <label htmlFor="end-date-sales" className="text-sm font-medium text-slate-600">To:</label>
                    <input
                        id="end-date-sales"
                        type="date"
                        value={customEndDate}
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody>
                        {filteredLogs.length > 0 ? filteredLogs.map(log => {
                            const details = (log.details as any).submittedData ?? log.details;
                            const fileName = details.file_name || 'Sales Upload';
                            const rowCount = (details.rows || []).length || 0;
                            return (
